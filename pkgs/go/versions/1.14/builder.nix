...
, buildPackages, pkgsBuildTarget, fetchpatch }:
...
in stdenv.mkDerivation rec {
  ...
  patches = [
    (file "remove-tools-1.11.patch")
    (file "ssl-cert-file-1.13.patch")
    (file "remove-test-pie-1.14.patch")
    (file "creds-test.patch")
    (file "go-1.9-skip-flaky-19608.patch")
    (file "go-1.9-skip-flaky-20072.patch")
    (file "skip-external-network-tests.patch")
    (file "skip-nohup-tests.patch")

    # fix rare TestDontCacheBrokenHTTP2Conn failure
    (fetchpatch {
      url =
        "https://github.com/golang/go/commit/ea1437a8cdf6bb3c2d2447833a5d06dbd75f7ae4.patch";
      sha256 = "1lyzy4nf8c34a966vw45j3j7hzpvncq2gqspfxffzkyh17xd8sgy";
    })
  ] ++ [
    # breaks under load: https://github.com/golang/go/issues/25628
    (if stdenv.isAarch32 then
      (file "skip-test-extra-files-on-aarch32-1.14.patch")
    else
      (file "skip-test-extra-files-on-386-1.14.patch"))
