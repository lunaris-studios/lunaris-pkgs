...
{ name, version, parent, dir, sha256 }@args:
...
let
  ...
  go_bootstrap = buildPackages.callPackage (nixpkgs_file "bootstrap.nix") { };
...
in stdenv.mkDerivation rec {
  inherit name version parent dir sha256;
  ...
  depsTargetTargetPropagated =
    optionals stdenv.isDarwin [ Security Foundation ];
  ...
  patches = [
    (nixpkgs_file "remove-tools-1.11.patch")
    (nixpkgs_file "ssl-cert-file-1.16.patch")
    (nixpkgs_file "remove-test-pie-1.15.patch")
    (nixpkgs_file "creds-test.patch")
    (nixpkgs_file "go-1.9-skip-flaky-19608.patch")
    (nixpkgs_file "go-1.9-skip-flaky-20072.patch")
    (nixpkgs_file "skip-external-network-tests-1.16.patch")
    (nixpkgs_file "skip-nohup-tests.patch")
    (nixpkgs_file "skip-cgo-tests-1.15.patch")
    (nixpkgs_file "go_no_vendor_checks-1.16.patch")
  ] ++ [
    # breaks under load: https://github.com/golang/go/issues/25628
    (if stdenv.isAarch32 then
      (nixpkgs_file "skip-test-extra-files-on-aarch32-1.14.patch")
    else
      (nixpkgs_file "skip-test-extra-files-on-386-1.14.patch"))
  ];
