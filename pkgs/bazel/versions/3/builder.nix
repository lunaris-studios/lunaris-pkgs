...
let
  ...
  srcDepsSet = let
    srcs = (builtins.fromJSON
      (builtins.readFile (packageFile "bazel_3/src-deps.json")));
    toFetchurl = d:
...
in stdenv.mkDerivation rec {
  ...
  patches = [
    ...
    (packageFile "trim-last-argument-to-gcc-if-empty.patch")
    ...
    (packageFile "bazel_3/no-arc.patch")
    ...
    (substituteAll {
      src = (packageFile "strict_action_env.patch");
    ...
    (substituteAll {
      src = (packageFile "bazel_rc.patch");
      bazelSystemBazelRCPath = bazelRC;
    })
  ] ++ lib.optional enableNixHacks (packageFile "nix-hacks.patch");
  ...
  in (if !stdenv.hostPlatform.isDarwin then {
    ...
    shebang = callPackage (packageFile "shebang-test.nix") {
      inherit runLocal extracted bazelTest distDir;
    };
  } else
    { }) // {
      bashTools = callPackage (packageFile "bash-tools-test.nix") {
        inherit runLocal bazelTest distDir;
      };
      cpp = callPackage (packageFile "cpp-test.nix") {
        inherit runLocal bazelTest bazel-examples distDir;
      };
      java = callPackage (packageFile "java-test.nix") {
        inherit runLocal bazelTest bazel-examples distDir;
      };
      protobuf = callPackage (packageFile "protobuf-test.nix") {
        inherit runLocal bazelTest distDir;
      };
      pythonBinPath = callPackage (packageFile "python-bin-path-test.nix") {
        inherit runLocal bazelTest distDir;
      };

      bashToolsWithNixHacks = callPackage (packageFile "bash-tools-test.nix") {
        inherit runLocal bazelTest distDir;
        bazel = bazelWithNixHacks;
      };

      cppWithNixHacks = callPackage (packageFile "cpp-test.nix") {
        inherit runLocal bazelTest bazel-examples distDir;
        bazel = bazelWithNixHacks;
      };
      javaWithNixHacks = callPackage (packageFile "java-test.nix") {
        inherit runLocal bazelTest bazel-examples distDir;
        bazel = bazelWithNixHacks;
      };
      protobufWithNixHacks = callPackage (packageFile "protobuf-test.nix") {
        inherit runLocal bazelTest distDir;
        bazel = bazelWithNixHacks;
      };
      pythonBinPathWithNixHacks =
        callPackage (packageFile "python-bin-path-test.nix") {
          inherit runLocal bazelTest distDir;
          bazel = bazelWithNixHacks;
        };
  ...
  passthru.updater = writeScript "update-bazel-deps.sh" ''
    ...
    cat ${
      runCommand "bazel-deps.json" { } ''
        ...
        ${python3}/bin/python3 ${
          (packageFile "update-srcDeps.py")
        } ./WORKSPACE > $out
      ''
    } > ${builtins.toString (packageFile "bazel_3/src-deps.json")}
  '';
